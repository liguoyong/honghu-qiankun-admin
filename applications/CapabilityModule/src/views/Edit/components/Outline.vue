<template>
  <Sidebar ref="sidebar" :title="$t('outline.title')">
    <el-tree class="outlineTree" :class="{ isDark: isDark }" :data="treeData" :props="defaultProps"
...
const defaultProps = ref({
  label(treeData) {
    return treeData.data.richText ? treeData.data.text : treeData.data.text.replaceAll(/\n/g, '</br>')
  }
})
...
onMounted(() => {
  bus.on('data_change', data2 => {
    ...
    treeData.value = [props.mindMap.renderer.renderTree]
  })
})
