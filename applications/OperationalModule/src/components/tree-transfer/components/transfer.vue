      ...
      <div>
        <div class="transfer-left">
          ...
          <div class="transfer-main">
            <slot name="from"></slot>
            <!-- <el-input v-if="filter" clearable size="small" :placeholder="placeholder" v-model="filterFrom"
            class="filter-tree"></el-input> -->
            <el-tree ref="from-tree" v-show="self_from_data.length" show-checkbox :lazy="lazy" :indent="indent"
              ...
              :default-expanded-keys="from_expanded_keys" @check="fromTreeChecked" @node-drag-start="nodeDragStartLeft"
              @node-drag-enter="nodeDragEnterLeft" @node-drag-leave="nodeDragLeaveLeft" @node-drag-over="nodeDragOverLeft"
              @node-drag-end="nodeDragEndLeft" @node-drop="nodeDropLeft">
            ...
            <div class="nodata" v-show="!self_from_data.length">
              <img src="../assets/images/notreedata.png" alt="" srcset="">
              <span>暂无选中项</span>
            </div>
        ...
        <div class="transfer-center">
          ...
          <template v-else>
            <p class="transfer-center-item">
              <el-button @click="addToAims(true)" icon="el-icon-arrow-right" :disabled="from_disabled"></el-button>
            </p>
            <p class="transfer-center-item">
              <el-button @click="removeToSource" :disabled="to_disabled" icon="el-icon-arrow-left"></el-button>
            </p>
          </template>
        </div>
        <!-- 右侧穿梭框 目标框 -->
        <div class="transfer-right">
          ...
          <div class="transfer-main">
            ...
            <el-tree slot="to" ref="to-tree" v-show="self_to_data.length" show-checkbox :indent="indent" :draggable="draggable" :allow-drag="allowDrag"
              ...
              @node-drag-leave="nodeDragLeaveRight" @node-drag-over="nodeDragOverRight" @node-drag-end="nodeDragEndRight"
            ...
            <div class="nodata" v-show="!self_to_data.length">
              <img src="../assets/images/notreedata.png" alt="" srcset="">
              <span>暂无选中项</span>
            </div>
...
export default {
  ...
  watch: {
    ...
    from_check_keys(val) {
      if (val.length > 0) {
        ...
        let allCheck = false;
        if (!this.checkStrictly) {
          const roots = val.filter((item) => item[this.parentId] === this.rootParentIdValue);
  ...
  methods: {
    ...
    addToAims(emit) {
      ...
      let parentId__ = this.parentId || "parentId";
      let id__ = this["node_key"] || "id";
      let root__ = this.rootParentIdValue || 0;
      ...
      if (this.checkStrictly) {
        ...
        this.checkStrictlyAdd(arrayCheckedNodes, { children__, parentId__, id__, root__ });
      } else {
        ...
        arrayHalfCheckedNodes.forEach((i) => {
          ...
          let _parent = root__ !== i[parentId__] ? i[parentId__] : null;
        ...
        notInTargetNodes.forEach((i) => {
          let parentInHere = notInTargetNodes.some((t) => t[id__] === i[parentId__]);
    ...
    removeToSource() {
      ...
      if (this.checkStrictly) {
        ...
        this.checkStrictlyRemove(arrayCheckedNodes, { children__, parentId__, id__, root__ });
      } else {
        // 第一步：排除在对面已经存在的半选节点，然后将需穿梭半选节点的children设置为[]并穿梭;
        arrayHalfCheckedNodes.forEach((i) => {
     ...
     * @param {Object} options 配置项{ children__, parentId__, id__, root__ }
     */
    checkStrictlyAdd(nodes, options) {
      ...
      const assembly_data = new_nodes.reduce((pre, item, idx, arr) => {
        const find_parent = arr.find((i) => i[options.id__] == item[options.parentId__]);
        // 没找到父节点，将节点保留
      ...
      assembly_data.forEach((i) => {
        this.$refs["to-tree"].append(i, i[options.parentId__]);
      });
    ...
    deepFindParent(node, options) {
      if (node[options.parentId__] === options.root__) {
        this.$refs["from-tree"].append(node);
        return;
      }
      const parentInThere = this.from_array_clone.some(
        (i) => i[options.id__] === node[options.parentId__]
      );
      if (parentInThere) {
        this.$refs["from-tree"].append(node, node[options.parentId__]);
      } else {
     ...
     * @param {Object} options 字段名配置项{ children__, parentId__, id__, root__ }
     ...
     * @param {options} options 字段名配置项{ children__, parentId__, id__, root__ }
    ...
    /* findParentInTarget(item, options, from_data, to_data, from_ref, to_ref, isAdd) {
      ...
      const parentInThere = to_data.some((t) => t[options.id__] === item[options.parentId__]);
      const parentTransferred = this.strictly_transferred.some(
        (t) => t[options.id__] === item[options.parentId__]
      );
      if (parentInThere || parentTransferred) {
        this.$refs[to_ref].append(item, item[options.parentId__]);
      ...
      // 父节点不在对面
      // 当此节点是根节点时，直接穿梭
      if (item[options.parentId__] === options.root__) {
        this.$refs[to_ref].append(item);
        return;
      }
      // 当此节点也不根节点，先从本侧数据找到父节点，再看父节点的parentId是否在对面，或者直到找到根祖先直接穿梭
      const parent_node = from_data.find((t) => t[options.id__] === item[options.parentId__]);
    ...
    // 折叠所有的节点
    collapseAllNodes(ref) {
      console.log(this.$refs[ref]);
      const data = (ref === 'from-tree' ? this.self_from_data : this.self_to_data)
      this.$nextTick(() => {
